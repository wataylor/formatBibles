...
public class WorkbookManager {
  ...
  public CellStyle badIntegerSty;
  ...
  public void makeFormatters() {
    ...
    badIntegerSty = wb.createCellStyle();
    badIntegerSty.setDataFormat(format.getFormat("#"));
    badIntegerSty.setFillForegroundColor(IndexedColors.RED.index);
    badIntegerSty.setFillBackgroundColor(IndexedColors.RED.index);
   ...
   * @param name identify the column
   * @param wid width in characters
   */
  public void setNamedColumnWidth(String name, int wid) {
    Integer i = columnMap.get(name);
    if (i == null) { return; }
    sheet.setColumnWidth(i, wid * 256);
  }
  ...
  public boolean isSheetOK(String[] requiredColNames, int minRows) {
    ...
    if ((row == null) || (physicalRows < minRows)) {
	  ...
	  (PUTs.isStringMTP(fileName) ? "" : " in " + fileName) + " must have " + minRows +
	  " or more rows<br>\n");
      return false;
    }
    ...
    if (badCol != null) {
	  ...
	  (PUTs.isStringMTP(fileName) ? "" : " in " + fileName) +
  ...
  public void setBoolAndDirty(String col, boolean what, boolean dirtyIn){
    ...
    setMappedCellStringValue(col, want, (PUTs.isStringMTP(got) ? null : headerTextStyle), false);
  ...
  public void setInteger(String col, int want) {
  ...
  public void setIntegerAndDirty(String col, int want, boolean dirtyIn) {
    ...
    Cell cell;
    if (got == want) { // Blank columns return 0 as a value
      if (want == 0) { // Set it explicitly just in case.
	cell = row.getCell(columnMap.get(col), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK);
	cell.setCellValue(want);
      }
      return;
    }
    ...
    cell.setCellStyle(badIntegerSty);
    cell.setCellValue(want);
    if (dirtyIn) { whinge(col); }
  }
   ...
   * @param col name of the column
   * @param val value to set in the cell
   * @param nullOK If true it is OK if the cell is missing
   */
  public void setColumnTo(String col, String val, boolean nullOK) {
    Integer which = columnMap.get(col);
    if (which == null) {
      if (nullOK) { return; } // Do nothing - column is missing but that is OK}
      throw new RuntimeException("Trying to set missing column " + col);
    }
  ...
  public void setMappedCellStringValue(String col, String value) {
    setMappedCellStringValue(col, value, null, false);
  }
   ...
   * @param nullOK If true, it is OK if the column name is not found
   */
  public void setMappedCellStringValue(String col, String value,
      CellStyle cellStyle, boolean nullOK) {
    Integer which = columnMap.get(col);
    if (which == null) {
      if (nullOK) { return; } // Do nothing - column is missing but that is OK}
      throw new RuntimeException("Trying to set missing column " + col);
